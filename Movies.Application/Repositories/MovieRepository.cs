...
public class MovieRepository : IMovieRepository
{
    ...
    public async Task<bool> DeleteByIdAsync(Guid id, CancellationToken token = default)
    {
        ...
        var result = await connection.ExecuteAsync(new CommandDefinition("delete from movies where id = @id", 
            new { id }, cancellationToken: token));
    ...
    {
        using var connection = await _dbConnectionFactory.CreateConnectionAsync(token);
        return await connection.ExecuteScalarAsync<bool>(new CommandDefinition("select count(1) from movies where id = @id",
            new { id }, cancellationToken: token ));
    }
}